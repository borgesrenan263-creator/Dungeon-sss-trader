...
async function killMonster({ playerId, monsterId }) {
  ...
  const playerResult = db.exec(
    `SELECT id, nickname, class, level, xp FROM players WHERE id = ?;`,
    [Number(playerId)]
  );
  ...
  const monsterResult = db.exec(
    `SELECT id, name, sector, hp, attack FROM monsters WHERE id = ?;`,
    [Number(monsterId)]
  );
  ...
  db.run(
    `INSERT INTO combat_logs (player_id, monster_id, crystal_rank, gold_earned)
     VALUES (?, ?, ?, ?);`,
    [Number(playerId), Number(monsterId), drop.rank, drop.gold]
  );
...
async function getCombatLogs(playerId) {
  ...
  const result = db.exec(
    `SELECT id, player_id, monster_id, crystal_rank, gold_earned, created_at
     FROM combat_logs
     WHERE player_id = ?
     ORDER BY id DESC;`,
    [Number(playerId)]
